...
import { BookOpen, Plus, Settings, RefreshCw, CheckCircle, AlertCircle, ExternalLink, Database, FileText, Calendar, Users, Zap, Download, Upload, FolderSync as Sync, Clock, Filter, Search } from 'lucide-react';
          ...
          <div className="border-b border-gray-200">
            <nav className="flex space-x-8">
              {[
                ...
                { id: 'databases', label: `Databases (${databases.length})`, icon: FileText },
                ...
                {databases.length === 0 ? (
                  ...
                  <div className="space-y-3">
                    {databases.slice(0, 3).map((database) => (
                      <div key={database.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                        <div className="flex items-center space-x-3">
                          <div className="p-2 bg-blue-100 rounded-lg">
                          ...
                          <div>
                            ...
                            <p className="text-sm text-gray-600">{database.itemCount} items</p>
                          </div>
                        </div>
                        <div className="flex items-center space-x-2">
                          <span className={`w-2 h-2 rounded-full ${
                            database.isConnected ? 'bg-green-500' : 'bg-red-500'
                          }`}></span>
                    ...
                    {databases.length > 3 && (
                      ...
                      >
                        View all {databases.length} databases
                      </button>
                    )}
                  </div>
                )}
              ...
              {databases.length === 0 ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {databases.map((database) => (
                    <div key={database.id} className="bg-white rounded-xl border border-gray-200 p-6 hover:shadow-lg transition-all duration-200">
                      <div className="flex items-center justify-between mb-4">
                        <div className="p-3 bg-blue-100 rounded-lg">
                          <Database className="w-6 h-6 text-blue-600" />
                        </div>
                      ...
                      <h3 className="font-semibold text-gray-900 mb-2">{database.name}</h3>
                      <p className="text-gray-600 text-sm mb-4">{database.description}</p>
                      
                      <div className="space-y-2 mb-4">
                        <div className="flex justify-between text-sm">
                          <span className="text-gray-600">Items:</span>
                          <span className="font-medium">{database.itemCount}</span>
                        </div>
                        <div className="flex justify-between text-sm">
                          <span className="text-gray-600">Last Sync:</span>
                          <span className="font-medium">
                            {database.lastSync ? new Date(database.lastSync).toLocaleDateString() : 'Never'}
                          </span>
                        </div>
                      </div>
                ...
                <div className="flex items-center justify-between mb-4">
                  ...
                  <div className="flex items-center space-x-2">
                    <select className="border border-gray-300 rounded-lg px-3 py-2 text-sm">
                      ...
                      <option>Content Database</option>
